...
public class ChatController {
    ...
    public ResponseEntity<GenericResponse<?>> sendMessage(@PathVariable("username") final String username,@RequestBody SendMessageRequest sendMessageRequest) {
        ...
        chatService.sendMessage(sendMessageRequest.getTo(),username,sendMessageRequest.getText(),sendMessageRequest.getType());
    ...
    @ApiResponse
    public ResponseEntity<GenericResponse<?>> fetchGroupChatHistory(@RequestParam("group") String group) {
        return ResponseEntity.ok(GenericResponse.builder().status(Status.SUCCES.getName()).data(chatService.fetchGroupChatHistory(group)).build());
    }
